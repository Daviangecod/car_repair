...
if(strtolower($_SERVER['REQUEST_METHOD']) !== "post") {
    ...
    redirect(baseUrl('admin/profile.php'), ['error' => 'method_not_allowed']);
}
else {

    if(empty($_POST['oldPassword']) || empty($_POST['newPassword']) || empty($_POST['confirmPassword'])) {
        setFlashMessage('error', 'One or More Fields are Empty');
        redirect(baseUrl('admin/profile.php'), ['error' => 'empty_fields']);
    }
    else {
        ...
        if($newPassword !== $confirmPassword) {
            ...
            redirect(baseUrl('admin/profile.php'), ['error' => 'incorrect_password']);
        }
        ...
        if(mysqli_num_rows($checkResult) == 1) { 
            ...
            if(password_verify($oldPassword, $check['password'])) {
                ...
                if($result) {
                    ...
                    redirect(baseUrl('admin/profile.php'), ['success' => 'password_update_success']);
                }
                else {
                    setFlashMessage('error', 'User Password Update Failed');
                    redirect(baseUrl('admin/profile.php'), ['error' => 'password_update_failed']);
                }
            ...
            redirect(baseUrl('admin/profile.php'), ['error' => 'unexpected_error']);
        }
