...
if(strtolower($_SERVER['REQUEST_METHOD']) !== "post" || empty($_POST['id'])) {
    ...
    redirect(baseUrl('admin/booking/index.php'), ['error' => 'method_not_allowed']);
}
else {
    $bookingId = $_POST['id'];
    ...
    if(empty($_POST['password'])) {
        ...
        redirect(baseUrl('admin/booking/index.php'), ['error' => 'empty_fields']);
    }
    else {
            ...
            if($result) {
                ...
                if(password_verify($password, $data['password'])) {
                    ...
                    if($result) {
                        ...
                        redirect(baseUrl('admin/booking/index.php'), ['success' => 'booking_delete_success']);
                    }
                    else {
                        setFlashMessage('error', 'Booking Deletion Failed');
                        redirect(baseUrl('admin/booking/index.php'), ['error' => 'booking_delete_failed']);
                    }

                }
                else {
                        setFlashMessage('error', 'Invalid Password, Validation Failed');
                        redirect(baseUrl('admin/booking/index.php'), ['validate' => '0']);
                }

            }
            else {
                setFlashMessage('error', 'Unexpected Error');
                redirect(baseUrl('admin/booking/index.php'), ['error' => 'unexpected_error']);
            }
