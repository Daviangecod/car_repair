...
if (isset($_GET['id'])):
    ...
    $shopLanguage = [];
    $shopLanguageId = $_GET["id"];

    $query = "SELECT * FROM languages WHERE id = $shopLanguageId";
    $result = mysqli_query($connection, $query);

    if (mysqli_num_rows($result) == 1):
        $shopLanguage = mysqli_fetch_assoc($result);
?>
    ...
    <div id="layoutSidenav_content" class="bg-light">
        <main>
            <div class="container px-4">
                ...
                <div>
                    <form action="<?= baseUrl('mechanic/language/action/update.php') ?>" method="POST" enctype="multipart/form-data">

                        <input type="hidden" name="id" value="<?= $shopLanguage['id'] ?>" />
                        ...
                        <div class="col-12">
                                ...
                                <select name="language" id="language" class="form-select" required>
                                    ...
                                    <?php
                                    if (file_exists(basePath("/json/languages.json"))) {

                                        $file = file_get_contents(basePath('/json/languages.json'));
                                    ...
                                    <?php if (isset($languages)) : ?>
                                        ...
                                        <?php foreach ($languages as $language): ?>
                                            ...
                                            <?php if($shopLanguage['name'] == $language->name): ?>
                            ...
                            <div class="col-12">
                                <label for="shop" class="fw-bold">Select Shop <small><em>(The shop that uses the language)</em></small> <span class="text-danger">*</span></label>
                                <select name="shop" id="shop" class="form-select" required>
                                    ...
                                    <?php if (count($shops) > 0) : ?>
                                        ...
                                        <?php foreach ($shops as $shop): ?>
                                            ...
                                            <?php if($shopLanguage['shop_id'] == $shop['id']): ?>
