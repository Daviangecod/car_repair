<?php $pageTitle = "Modify Payment Type"; ?>
...
if (isset($_GET['id'])):
    ...
    $shopPaymentType = [];
    $shopPaymentTypeId = $_GET["id"];

    $query = "SELECT * FROM payment_types WHERE id = $shopPaymentTypeId";
    $result = mysqli_query($connection, $query);

    if (mysqli_num_rows($result) == 1):
        $shopPaymentType = mysqli_fetch_assoc($result);
?>
    ...
    <div id="layoutSidenav_content" class="bg-light">
        <main>
            <div class="container px-4">
                ...
                <div>
                    <form action="<?= baseUrl('mechanic/payment/action/update.php') ?>" method="POST" enctype="multipart/form-data">

                        <input type="hidden" name="id" value="<?= $shopPaymentType['id'] ?>" />
                                ...
                                <select name="paymentType" id="paymentType" class="form-select" required>
                                    ...
                                    <?php if (isset($paymentTypes)) : ?>
                                        ...
                                        <?php foreach ($paymentTypes as $paymentType): ?>
                                            ...
                                            <?php if($shopPaymentType['name'] == $paymentType->name): ?>

                                                <option value="<?= $paymentType->name ?>" selected><?= $paymentType->name ?></option>
                                ...
                                <select name="shop" id="shop" class="form-select" required>
                                    ...
                                    <?php if (count($shops) > 0) : ?>
                                        ...
                                        <?php foreach ($shops as $shop): ?>
                                            ...
                                            <?php if($shopPaymentType['shop_id'] == $shop['id']): ?>
